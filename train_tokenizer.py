...
from SAMPO.vq_model import CompressiveVQModel, Discriminator, LPIPS
...
def parse_args():
    ...
    parser.add_argument("--log_image_steps", type=int, default=5000)
...
def main():
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S", 
        level=logging.INFO,
        filename=os.path.join(args.output_dir, 'train_tokenizer.log'),
    )
    ...
    if args.use_ema:
        ema_model = EMAModel(model.parameters(), model_cls=CompressiveVQModel, model_config=model.config)
