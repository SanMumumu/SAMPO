...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.using_xform = False# flash_if_available and memory_efficient_attention is not None  # False
...
class AdaLNSelfAttn(nn.Module):
    def __init__(
        ...
        self.ln1 = norm_layer(embed_dim, elementwise_affine=True)
        self.ln2 = norm_layer(embed_dim, elementwise_affine=True)
    ...
    # NOTE: attn_bias is None during inference because kv cache is enabled
    def forward(self, x, attn_bias, cond_BD=None):
        if cond_BD is None:
            x = x + self.drop_path(self.attn(self.ln1(x), attn_bias=attn_bias))
...
class AdaLNBeforeHead(nn.Module):
    ...
    def forward(self, x_BLC, cond_BD=None):
        if cond_BD is None:
