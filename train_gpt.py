...
def parse_args():
    ...
    parser.add_argument('--cotracker_prob', default=0.5, type=int)
    ...
    args.model_type = args.config_name.split('/')[-2]
...
def evaluate(args, accelerator, tokenizer, model, eval_dataloader, evaluator, completed_steps):
    ...
    for i, batch in enumerate(eval_dataloader):
        ...
        prefix, dyn, indices_c, indices_d = accelerator.unwrap_model(tokenizer).tokenize(pixel_values,
                                                                      args.context_length,
                                                                      )
        model_input = {
                    'prefix': prefix,
        ...
        if (i % args.log_gif_interval == 0 and accelerator.is_main_process) or args.use_frame_metrics or args.use_fvd:
            recon_BT3HW =  accelerator.unwrap_model(model).autoregressive_infer_cfg(prefix, dyn, pixel_values.shape[0], args.seed)
        ...
        if i % args.log_gif_interval == 0 and accelerator.is_main_process:
            ...
            gt_tensor = tensor_to_grid(pixel_values)   # [16, 64, 64, 3]
            recon_tensor = tensor_to_grid(recon_BT3HW) # [16, 64, 64, 3]

            def stack_images(tensor, row_size=16):
                images = [Image.fromarray(frame.numpy()) for frame in tensor]
            ...
            # row_gt = stack_images(gt_tensor[0,:].unsqueeze(0)) # TODO:NEW
            row_gt = stack_images(gt_tensor)
            row_recon = stack_images(recon_tensor)
            combined = Image.new("RGB", (row_gt.width, row_gt.height * 2))
            combined.paste(row_gt, (0, 0))
            combined.paste(row_recon, (0, row_gt.height))
            combined.save(f"{save_path}/val-samples-{completed_steps}-{i}.png")
            if not args.use_frame_metrics:
                assert pixel_values.shape[0] == recon_BT3HW.shape[0]
                mse_values.append(torch.mean((pixel_values - recon_BT3HW) ** 2).repeat(batch_size))

        if args.use_fvd:
            detector_kwargs = dict(rescale=True, resize=True, return_features=True)
            images = recon_BT3HW.permute(0, 2, 1, 3, 4).contiguous()  # [batch_size, c, t, h, w]
            if args.max_decode_batchsize is not None and images.shape[0] > args.max_decode_batchsize:
                features = batch_forward(
                    args.max_decode_batchsize,
                    images * 255.,
            ...
            if accelerator.is_main_process:
                ...
                logger.info(f"current fvd: {accelerator.unwrap_model(evaluator).compute_fvd(real_feats, gen_feats)}")

        if args.use_frame_metrics:
            mse_value, psnr_value, ssim_value, lpips_value = evaluator(pixel_values.clamp( # TODO:NEW
                0.0, 1.0), recon_BT3HW)
...
def start_train():
    ...
    logging.basicConfig(
        ...
        datefmt="%m/%d/%Y %H:%M:%S",
        level=logging.INFO,
        filename=os.path.join(args.output_dir, 'train_gpt.log'),
    )
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if accelerator.sync_gradients and accelerator.is_main_process:
                ...
                if completed_steps % checkpointing_steps == 0 and avg_loss < 20.0:
                    output_dir = os.path.join(args.output_dir, f"checkpoints/checkpoint_{completed_steps}")
